...
function sl_enqueue_scripts() {
	...
	wp_enqueue_script( 'simple-likes-public-js', get_template_directory_uri() . '/js/simple-likes-public.js', array( 'mybtsarmy-jquery' ), '0.5', false );
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => __( 'Like', 'mybtsarmy' ),
		'unlike' => __( 'Unlike', 'mybtsarmy' )
	) ); 
}
/**
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'mybtsarmy' ) );
	}
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'mybtsarmy' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'mybtsarmy' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
